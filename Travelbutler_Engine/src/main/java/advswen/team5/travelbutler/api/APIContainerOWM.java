...
public class APIContainerOWM implements IAPIContainer {
    ...
    currentMinTemperatur = cwd.getMainInstance().getMinTemperature();
    currentMaxTemperatur = cwd.getMainInstance().getMaxTemperature();
    currentHumidity = cwd.getMainInstance().getHumidity();
    currentPressure = cwd.getMainInstance().getPressure();
    
//    DailyForecast fw = owm.dailyForecastByCityName(requestString, (byte) 5);
//    for(int i=0;i<=5;i++){
//    	forecastTempMin.add(i, fw.getForecastInstance(i).getTemperatureInstance().getMinimumTemperature());	
//    	forecastTempMax.add(i, fw.getForecastInstance(i).getTemperatureInstance().getMaximumTemperature());	
//    	System.out.println("Generell: " + fw.getForecastInstance(i).getWeatherInstance(0).getWeatherDescription());
